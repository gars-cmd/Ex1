    ...
    if rangeElev==1:
        ...
        for elevator in range(rangeElev):
            if elevator<(rangeElev/2):
                ...
                # print(elevator)
        global count0,count1
        # print(len(elevatorUp),len(elevatorDown))
        # print(Call.getType(c))
        if Call.getType(c)==1:
...
def writeOnCsv(fileToWriteOn , listcall:list):
    with open (fileToWriteOn , 'w' , newline= '') as f:
        ...
        for calls in listcall:
            writer.writerow(['Elevator call' , Call.getTime(calls) , Call.getSrc(calls) , Call.getDest(calls) , Call.allocatedTo(calls)])
...
if __name__ == "__main__":
    ...
    for item in listCall:
        ...
        # print(Call.getType(item) , Call.allocatedTo(item) )
        # print(Call.allocatedTo(item))
